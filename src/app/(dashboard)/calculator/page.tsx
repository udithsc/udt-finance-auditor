...
import { Calculator as CalcIcon, Calendar, Percent, Info, ChevronDown, ChevronUp } from "lucide-react";
        ...
        <div className="lg:col-span-5 bg-[#0052cc] rounded-3xl p-8 shadow-2xl flex flex-col gap-8 text-white">
          <div className="flex flex-col gap-2">
            ...
            <div className="relative group">
               <div className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-200 font-bold border-r border-blue-400/30 pr-4">LKR</div>
               <input 
                 type="number" 
                 value={loanAmount} 
               ...
               />
               <p className="text-[10px] text-blue-200 mt-2 text-right">Minimum LKR 500,000</p>
            </div>
          </div>
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col">
           <div className="p-10 flex flex-col items-center text-center">
              ...
              <p className="text-6xl font-black text-[#0052cc] mt-4 mb-2 tracking-tighter">
                Rs. {results?.monthlyInstallment.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
              ...
              <div className="w-full max-w-md mt-10 space-y-4">
                 <div className="flex justify-between items-center py-4 border-b border-zinc-100">
                    ...
                    <span className="font-bold text-zinc-800">LKR {results?.firstMonthPrincipal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                 </div>
                 <div className="flex justify-between items-center py-4 border-b border-zinc-100">
                    <span className="text-zinc-500 font-medium">Interest (1st Month)</span>
                    <span className="font-bold text-zinc-800">LKR {results?.firstMonthInterest.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                 </div>
              ...
              <div className="overflow-hidden rounded-xl border border-zinc-200">
                  ...
                  <tbody className="divide-y divide-zinc-200 bg-white">
                    {results?.breakdown.map((row) => (
                      <tr key={row.month} className="hover:bg-blue-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-zinc-800">LKR {row.principal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td className="px-4 py-3 text-zinc-800">LKR {row.interest.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td className="px-4 py-3 text-right font-bold text-[#0052cc]">LKR {row.installment.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td className="px-4 py-3 text-right text-zinc-500">LKR {row.balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
               ...
               <div className="mt-8 grid grid-cols-2 gap-4">
                  <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm">
                     ...
                     <p className="text-xl font-bold text-zinc-800">LKR {results?.totalInterest.toLocaleString()}</p>
                  </div>
                  <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm">
                     <p className="text-[10px] uppercase font-bold text-zinc-400 mb-1">Total Payment</p>
                     <p className="text-xl font-bold text-zinc-800">LKR {results?.totalPayment.toLocaleString()}</p>
                  </div>
               </div>
           </div>
        </div>
