...
export default async function Dashboard() {
  ...
  const totalDebts = debts.reduce((sum, d) => Math.round((sum + (d.remaining || 0)) * 100) / 100, 0);
  ...
  transactions.forEach(tx => {
    ...
    if (!tx.isInternal && txDate.getMonth() === currentMonth && txDate.getFullYear() === currentYear) {
      if (tx.type === "CREDIT") {
        realIncome = Math.round((realIncome + tx.amount) * 100) / 100;
      } else if (tx.type === "DEBIT") {
        realExpenses = Math.round((realExpenses + tx.amount) * 100) / 100;
      }
    }
  });
  ...
  transactions.forEach(tx => {
    ...
    if (tx.type === "CREDIT") {
       monthlyDataMap[monthYear].income = Math.round((monthlyDataMap[monthYear].income + tx.amount) * 100) / 100;
    } else if (tx.type === "DEBIT") {
       monthlyDataMap[monthYear].expenses = Math.round((monthlyDataMap[monthYear].expenses + tx.amount) * 100) / 100;
    }
  });
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-500 mt-1">Your financial life at a glance (Base Currency: MYR)</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button className="px-4 py-2 border border-white/10 rounded-xl hover:bg-white/5 transition-colors text-sm font-medium">
            Generate Report
          </button>
          <button className="px-4 py-2 bg-primary text-primary-foreground rounded-xl shadow-lg shadow-primary/20 hover:scale-105 transition-all text-sm font-medium">
            + New Record
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="glass rounded-2xl p-6 group">
          ...
          <div className="mt-4">
            ...
            <p className="text-3xl font-bold tracking-tight mt-1">RM {netWorth.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
          </div>
        </div>
        ...
        <div className="glass rounded-2xl p-6 group">
          ...
          <div className="mt-4">
            ...
            <p className="text-3xl font-bold tracking-tight mt-1">RM {totalAssets.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
        ...
        <div className="glass rounded-2xl p-6 group">
          ...
          <div className="mt-4">
            ...
            <p className="text-3xl font-bold tracking-tight mt-1">RM {totalDebts.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
        ...
        <div className="glass rounded-2xl p-6 group">
          ...
          <div className="mt-4">
            ...
            <p className="text-3xl font-bold tracking-tight mt-1">RM {monthlySavings.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="glass rounded-2xl p-6 flex flex-col">
          ...
          <div className="flex-1 flex flex-col gap-4">
            <div className="bg-white/5 border border-white/5 rounded-xl p-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-emerald-400 font-bold text-xl mt-1">RM {realIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
            ...
            <div className="bg-white/5 border border-white/5 rounded-xl p-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-red-400 font-bold text-xl mt-1">RM {realExpenses.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
          ...
          <div className="mt-6 border-t border-white/5 pt-6">
            ...
            <NetWorthBreakdown assets={totalAssets} debts={totalDebts} />
          </div>
        </div>
      </div>
    </div>
